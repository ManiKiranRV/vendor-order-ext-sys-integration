...
export class AddressValidation {
    ...
    async getDataAfterValidation(data:any,org:any) {
        ...
        if(org === process.env.ORG_NAME){
            sellerData = [data.find((item:any) => item.address_type === process.env.SELLER)];
            // this.logger.log("sellerData---->",sellerData)
            buyerData = [data.find((item:any) => item.address_type === process.env.BUYER)]
        ...
        if(sellerData.length>0){
            ...
            let afterValidationDetails:any = await this.validationForPostalAddressDetails(sellerData[0])
            ...
            if(afterValidationDetails.status === "Success"){
                ...
                this.logger.log("sellerDetails-------->",sellerDetails)
            }else{
                // Return Error
        ...
        if(buyerData.length>0){
            ...
            let afterValidationDetails:any = await this.validationForPostalAddressDetails(buyerData[0])
            ...
            if(afterValidationDetails.status === "Success"){
                ...
                this.logger.log("buyerDetails-------->",buyerDetails)
            }else{
                // Return Error
                return ({"status":"Error","data":afterValidationDetails.data})
            }
